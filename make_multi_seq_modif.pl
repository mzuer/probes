...
## slightly modified MZ to get another name of file
...
while($ll=<FASTA>) {
  if ($ll=~ /^>(\S+)/) {
    ...
    if (defined($cid)) {
      ...
      my $dirname = "clust".$cid.".fasta";	
      open(OUT, ">> $out_dir/$dirname") || die "can not open file to write $out_dir/$dirname";
			      #open(OUT, ">> $out_dir/$cid") || die "can not open file to write $out_dir/$cid";  # original version
      $outfile_open = 1;
      $flag = 1;
    }
    else {
